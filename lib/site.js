...
function show(el) {
  // A photograph that has not arrived yet is fetched now and revealed when it lands (or after 1.5s regardless),
  // rather than opening the mask on an empty frame.
  if (el.tagName === 'IMG' && !el.complete && !el._awaited) {
    el._awaited = true;
    if (el.loading === 'lazy') el.loading = 'eager';
    const go = () => { el.removeEventListener('load', go); el.removeEventListener('error', go); clearTimeout(cap); show(el); };
...
// Photograph loading. Every photograph below the fold is loading="lazy", but the browser decides when a lazy image
// is "near" with the same visibility test IntersectionObserver uses — and a mask reveal clips its image away
// entirely, so the download only began once the reveal had already opened on an empty frame. Here each lazy image is
// watched through its frame (never clipped) and switched to eager two screens before it arrives; once the page has
// loaded and gone quiet the rest are fetched two at a time, top to bottom, so nothing is left to load on arrival.
// Images inside a display:none branch (the other breakpoint's copy of a scene) are left alone.
let imageIO = null;
const framed = new WeakMap(); // frame → lazy images inside it
const fetchNow = (img) => { if (img.loading === 'lazy') img.loading = 'eager'; };
const rendered = (img) => img.getClientRects().length > 0 || (img.parentElement && img.parentElement.getClientRects().length > 0);
function warmRest() {
  const queue = [...document.querySelectorAll('img[loading="lazy"]')].filter(rendered);
  const next = () => {
    const img = queue.shift();
    if (!img) return;
    if (img.loading !== 'lazy' || !img.isConnected) return next();
    const done = () => { img.removeEventListener('load', done); img.removeEventListener('error', done); clearTimeout(timer); next(); };
  ...
  if (!imageIO) {
    imageIO = new IntersectionObserver((entries) => {
      for (const e of entries) {
        ...
        (framed.get(e.target) || []).forEach(fetchNow);
      }
    }, { rootMargin: '200% 0px 200% 0px' });
  }
  for (const img of document.querySelectorAll('img[loading="lazy"]:not([data-img-ready])')) {
    img.dataset.imgReady = '1';
    const frame = img.parentElement || img;
    if (!framed.has(frame)) framed.set(frame, []);
    framed.get(frame).push(img);
    imageIO.observe(frame);
  }
  // Visitors who asked to save data, or are on a slow connection, only get what they scroll to.
  const net = navigator.connection;
  if (net && (net.saveData || /2g|3g/.test(net.effectiveType || ''))) return;
  clearTimeout(warmTimer);
  const start = () => { warmTimer = setTimeout(warmRest, 1200); };
